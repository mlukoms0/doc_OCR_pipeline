...
--   3) After extract.py has written JSON to gs://<bucket>/json/, load it
--      (command line):
--        bq load --source_format=NEWLINE_DELIMITED_JSON --ignore_unknown_values \
--          144240581301:freight.stg_extractions "gs://<bucket>/json/*.json"
...
CREATE TABLE IF NOT EXISTS `144240581301.freight.stg_extractions` (
  ...
  documents ARRAY<STRUCT<
    doc_type STRING, page_range STRING, confidence FLOAT64, ocr_confidence FLOAT64, illegible BOOL,
    load_number STRING, pro_number STRING, bol_number STRING, order_number STRING,
    ...
    commodity STRING, weight FLOAT64, weight_unit STRING, pieces INT64,
...
CREATE TABLE IF NOT EXISTS `144240581301.freight.documents` (
  ...
  commodity STRING, weight NUMERIC, weight_unit STRING, pieces INT64,
  equipment_type STRING, freight_class STRING, hazmat BOOL,
  delivered BOOL, received_by STRING, signature_present BOOL, exceptions_notes STRING,
  loaded_at TIMESTAMP
);
...
SELECT
  ...
  d.confidence,
  d.ocr_confidence,
  (d.confidence < 0.75 OR COALESCE(d.ocr_confidence, 0) < 0.6 OR d.illegible) AS needs_review,  -- tune thresholds after the pilot
  d.load_number, d.pro_number, d.bol_number, d.order_number,
  ...
  d.commodity, CAST(d.weight AS NUMERIC), d.weight_unit, d.pieces,
